"""
Energomonitor BESS - FINAL FIXED for Streamlit Cloud
- Relative paths BASE_DIR
- No st.rerun() - fixes removeChild NotFoundError
- ENTSO-E IBEX integration
"""
import sys
...
if "sim_self" not in st.session_state: st.session_state.sim_self=None
if "sim_price" not in st.session_state: st.session_state.sim_price=None

st.sidebar.title("⚡ Ценови данни - IBEX")
price_source = st.sidebar.radio("Източник", ["ENTSO-E Transparency (реални IBEX цени)", "Качи файл с IBEX цени (CSV/XLSX)", "Демо синтетични цени (IBEX-like)"], index=2, key="price_source_radio")
...
if "ENTSO-E" in price_source:
    ...
    entsoe_token = st.sidebar.text_input("ENTSO-E Security Token", type="password", key="entsoe_token_input")
    if not entsoe_token:
...
# FIXED: relative path for Streamlit Cloud
...
with c2:
    use_demo=st.button("🔥 Демо - малък излишък", use_container_width=True, key="demo_btn")
...
if use_demo:
    if demo_path.exists():
        ...
        st.error(f"Демо файл не е намерен: {demo_path}. Проверете data/ папката в repo-то.")
elif uploaded:
    ...
    with st.expander("Import Wizard + ENTSO-E", expanded=True):
        ...
        def sel(can,label,cont,req=False):
            with cont:
                ...
                s=st.selectbox(can,opts,index=idx,key=f"m_{can}_v2",label_visibility="collapsed")
                if s!="--":
                    mapping[can]=s
                    if "price" not in can:
                        u=st.selectbox(f"u_{can}",["kW","kWh","MW","MWh"],key=f"u_{can}_v2")
                        units[can]=u
        ...
        if all(k in mapping for k in ["timestamp","consumption","pv_production"]):
            if st.button("▶ Изчисли базов баланс + зареди IBEX цени", type="primary", key="calc_balance_btn"):
                with st.spinner("Зареждам данни и IBEX цени..."):
                    ...
                    st.success("Базов баланс изчислен + IBEX цени заредени!")
...
if st.session_state.baseline_df is not None:
    ...
    with ca:
        ...
        st.plotly_chart(fig, use_container_width=True, key="cons_pv_chart")
    with cb:
        ...
        st.plotly_chart(fig2, use_container_width=True, key="hourly_chart")
    ...
    with st.sidebar:
        st.divider()
        st.subheader("BESS")
        rated=st.slider("Капацитет kWh", 20, 1000, 100, step=10, key="rated_slider")
        c_ratio=st.slider("C-rate", 0.2, 1.0, 0.5, key="c_rate_slider")
        min_soc=st.slider("Min SOC %", 0, 30, 10, key="min_soc_slider")
        max_soc=st.slider("Max SOC %", 70, 100, 90, key="max_soc_slider")
        init_soc=st.slider("Init SOC %", 0, 100, 50, key="init_soc_slider")
        rt_eff=st.slider("Round-trip %", 80, 98, 90, key="rt_eff_slider")
        st.subheader("CAPEX")
        capex_kwh=st.number_input("€/kWh", value=350, key="capex_kwh_input")
        capex_kw=st.number_input("€/kW", value=150, key="capex_kw_input")
        install=st.number_input("Инсталация €", value=5000, key="install_input")
        st.subheader("Икономика")
        discount=st.slider("Дисконт %", 0, 12, 6, key="discount_slider")
        lifetime=st.slider("Живот години", 5, 25, 15, key="lifetime_slider")
        buy_price_fixed=st.number_input("Фикс покупка €/MWh", value=180, key="buy_fixed")
        sell_price_fixed=st.number_input("Фикс продажба €/MWh", value=90, key="sell_fixed")
    ...
    if st.button("▶ Симулирай", type="primary", key="simulate_btn"):
    ...
    if st.session_state.sim_self is not None and st.session_state.sim_price is not None:
        ...
        if fin_self.simple_payback_years>15:
            ...
            st.markdown(f'<div class="ok"><b>Извод:</b> Payback {fin_self.simple_payback_years:.1f}г, NPV {fin_self.npv_eur:,.0f}€</div>', unsafe_allow_html=True)
        
        st.divider()
        st.header("📋 Сравни оферта (D)")
        col_o1,col_o2,col_o3=st.columns(3)
        with col_o1:
            offer_cap=st.number_input("Оферта kWh", value=200, key="offer_cap")
            offer_power=st.number_input("Оферта kW", value=100, key="offer_power")
        with col_o2:
            offer_capex=st.number_input("CAPEX €", value=80000, key="offer_capex")
            offer_promised=st.number_input("Обещано MWh/год", value=15.0, key="offer_promised")
        with col_o3:
            offer_eff=st.slider("Ефективност %", 80, 98, 90, key="offer_eff")
        if st.button("📊 Сравни оферта", key="compare_offer_btn"):
        ...
        if "offer_comp" in st.session_state and st.session_state.offer_comp is not None:
            ...
            st.plotly_chart(fig_offer, use_container_width=True, key="offer_chart")
        
        st.divider()
        st.header("🤖 Експертен доклад + PDF")
        if st.button("📄 Генерирай доклад", key="gen_report_btn"):
        ...
        if "expert_report" in st.session_state and st.session_state.expert_report:
            st.markdown(st.session_state.expert_report)
            if st.button("⬇️ Генерирай PDF", key="gen_pdf_btn"):
            ...
            if "pdf_bytes" in st.session_state and st.session_state.pdf_bytes:
                st.download_button("📥 Свали PDF", data=st.session_state.pdf_bytes, file_name="BESS_IBEX_ENTSOE.pdf", mime="application/pdf", key="download_pdf_btn")
